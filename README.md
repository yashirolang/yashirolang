...
| 現在地 | v2（安全性・エラー処理・共有所有）実装済み／**RISC-V のベアメタルで動作**・531 テスト |
...
yashirolang --version          # → yashirolang 0.17.0 (stage0)
...
- **数える形のループがベクトル化されるように**。添字の範囲を
  ...
  512³ の行列積（平坦な `list[float]`）が **39.6 ms** — C の平坦配列 31.3 ms の
  1.27 倍まで来ました（以前は 6.4 倍）。
