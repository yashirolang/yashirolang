...
**注意: 粗さの方針**：`xs[0]` と `xs[1]` は同じ Place として扱います（保守的）。
...
**CFG は作りません。** yashirolang の制御構文は構造化されている（`goto` が無い）ので、
...
**注意: `rc[T]` を「所有型」として扱うので、ムーブ検査も drop 挿入もそのまま働きます。**
...
**コンパイラ自身が所有権検査を通ります**（`make own-strict`）。移行のときに
壊れた典型パターンと、その対策を残しておきます（新しいコードを書くときも同じです）：
...
（**コンパイラ自身が言語仕様のテストケース**）。実際、共有がどうしても要る
ところだけが `rc[T]` に移りました。
...
注意: **既定（借り）のまま `swap.N` を足したら、`--drop` で壊れました。**
...
解放が 1 回だけになります。番人は `tests/cases/drop_swap.ys`（ASan 付き）。
...
| 種類 | 置き場所 | 例 |
|---|---|---|
| 通るべきコード | `tests/cases/own_*.ys` | move → 再代入 → 使用 |
| 落ちるべきコード | `tests/cases/err_move_*.ys` | use-after-move、二重可変借用 |
| 解放の確認 | `tests/cases/drop_*.ys` | `drop` メソッドの呼ばれ方と順序を `print` で観測 |
