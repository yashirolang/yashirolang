...
⚠️ **`list[float]` に `+` は使えません。** `+` はリストの連結にも見えるため、
...
⚠️ FFT の入力長は **2 の冪**に限ります（そうでなければ panic）。
...
⚠️ **すべて自前実装です**（libc の `libm` を呼びません）。ベアメタルで動かすためです。
...
⚠️ **PNG は出しません**（zlib の実装が要るため）。画面に出す窓もありません。
...
⚠️ **列の型は取り出すときに選びます。** `df.text("price")` のように取り違えると
...
⚠️ `scope:` の外では、**渡した値は移動します**（渡したあとは触れません）。
中では借りを渡せます（下記）。
...
⚠️ `scope:` の**途中から抜けることはできません**（`return` など）。
...
⚠️ 生の `lock` / `unlock` はありません（解き忘れが起きない形にしてあります）。
⚠️ **集計にしか使わないでください。** 計算の本体をロックの中に入れると、
コアを増やしても速くなりません。

### 7.5 ⚠️ `async` / `await` はありません
