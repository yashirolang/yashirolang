...
| `from_code_point(cp: int) -> str` | コードポイントを UTF-8 の文字にする（Python の `chr` 相当）。注意: **組み込みの `chr()` はバイト**（0〜255）を作ります |
...
注意: `width` は**目安**です。端末や字体によって幅は変わります。
「よく全角で表示される範囲」を 2 桁として数えています。

> **注意: 桁を揃えるときは `len` ではなく `lpad` / `rpad` を使ってください。**
...
> **注意: 探索や切り出しはバイト単位です。** `str` は UTF-8 のバイト列で、
...
| `listdir(path: str) -> list[str]` | 中身の名前（`.` と `..` は含まない）。注意: **並びは OS 任せ** |
| `rmdir(path: str) -> bool` | **空の**ディレクトリを消す |
| `remove_all(path: str) -> bool` | 中身ごと消す（`rm -rf` 相当。注意: リンクの先はたどらない） |
...
注意: **区切りは `/` だけ**を見ます（Windows でも `/` で書きます）。
注意: `listdir` の並びは OS 任せです。**同じ結果が欲しいときは自分で整列**してください。
...
> **注意: ベアメタルでは使えません。** 標準入力は `runtime/hosted.c`（PC 用）にしかなく、
...
| `cpu_count() -> int` | 使える CPU コアの数（`spawn` の本数を決めるのに使う）。**0 は返りません**。注意: ベアメタルでは使えません |
...
注意: **鍵の比較は `==` です。** `int` / `float` / `str` は**中身**で、
...
注意: **鍵に使えるのは `hash` が定義された型**（`int` / `float` / `str` / `bool`）
...
注意: ハッシュは FNV-1a です。**暗号用ではありません。** 敵が鍵を選べる場面
（外部入力を鍵にするサーバなど）では、衝突を狙われて線形探索に落とされます。

注意: **`items()` のように「組のリスト」を返すことはできません。** 中身は表が
...
`d.has(k)` と同じものです。注意: `in` は**右辺（入れ物）の型**で決まります。

## math

**Python の `math` に相当します。** すべて `float` を受け取り `float` を返します。

> **注意: `libm` は呼びません。** ランタイムは libc に依存しない約束なので
...
注意: **関数です。** グローバル変数の初期化式はリテラルしか書けないためです。
...
注意: `round` は **0 から遠いほうへ**丸めます（C の `round` と同じ。
Python の偶数丸めではありません）。

| `round_to(x, n)` | 小数点以下 `n` 桁に丸める（**表示のため**） |

> **注意: `str(float)` は「元に戻る最短の表記」です**（`0.1 + 0.2` は
...
注意: **`float ** float` は演算子としては使えません**（`**` は `int` 専用）。
`math.pow` を使ってください。
...
注意: **`div` は 0 の要素があると実行時エラーです**（`x / 0.0` が
...
| **`a * b`** | **行列の積**（`matmul`） |
| `-a` | `mneg` |
| `a == b` / `a != b` | 形と全要素が等しいか（注意: 誤差は考慮しません） |
| `m[i, j]` / `m[i, j] = v` | `get` / `set`（範囲検査つき） |

> **注意: numpy とは違います。** numpy の `a * b` は要素ごとの積、`a @ b` が
> 行列の積です。yashirolang に `@` は無いので、数学の書き方に合わせて
> **`*` を行列の積**にしました。**要素ごとの積は `mmul(a, b)`** です。

> **注意: スカラー倍（`a * 2.0`）は書けません。** 多重定義は左辺の型だけで
...
注意: **標本と母集団のどちらを使うかは、データの性質で決まります。**
...
注意: 2019 年の SI 改定で、光速・プランク定数・電気素量・ボルツマン定数・
アボガドロ定数は**定義値**になりました。
...
| `G()` | 万有引力定数。注意: **測定値**で不確かさが大きい（2.2×10⁻⁵） |
...
| `simpson(f, a, b, n)` | シンプソン則。誤差は h⁴。注意: `n` は偶数 |
| `integrate(f, a, b, tol)` | 分割を自動で増やす。注意: **収束は保証しません** |

| 求根 | |
|---|---|
| `bisect(f, a, b)` | 二分法。遅いが**必ず収束**。注意: 両端で符号が違うこと |
...
| `minimize(f, a, b, tol)` | 黄金分割探索。注意: 区間内の極小が 1 つだけであること |
...
注意: 渡す関数は **`raises` しないもの**に限ります（関数型はエラーの受け渡しを表しません）。
...
def main() -> int:
    ...
    s: set.Set[str] = set.Set()      # 注意: 型引数は左辺の注釈が渡します
    s.add("x")
    return 0
```
...
| `items() -> list[T]` | 中身。注意: **並びは入れた順ではありません** |
...
注意: リテラル `{1, 2}` は書けません。注意: `|` `&` `-` の多重定義は入れていません
...
| `as_bool()` / `as_float()` / `as_int()` / `as_str()` | 取り出す。注意: **種類が違えば panic** |
| `len()` / `at(i)` | 配列 |
...
注意: **数はすべて `float` で持ちます**（JSON の数に整数・小数の区別が無いため）。
...
注意: **分解代入（`a, b = f()`）は書けません。** `p.first` / `p.second` を使います。

注意: **所有権を受け取ります**（`own`）。入れ物の中身を組にして返すことは
できません（借りたものを渡せないため）。
...
| `max_of(xs)` / `min_of(xs)` | 注意: 空なら `panic` |
...
> **注意: 速さを測るときは `monotonic_ns` を使ってください。**
> `now_ns`（実時刻）は **戻ることがあります**（NTP の補正・夏時間・
> 利用者が時計を直す）。測っている最中に戻ると経過時間が負になります。

> **注意: ベアメタル（`kernel/`）では使えません。** 時計は OS の持ち物なので、
...
| 演算子 | 意味 |
|---|---|
| `+` `-` `*` `/` | 複素数どうしの四則（注意: `/` は `mag2(o)` が 0 だと**実行時エラー**） |
| 単項 `-` | 符号反転 |
| `==` `!=` | 実部・虚部の一致（注意: 誤差は考慮しません。`isclose` を使ってください） |
...
> **注意: 複素数はオブジェクトです**（`int` / `float` のような「値」ではありません）。
...
> **注意: 長さは 2 のべき乗だけです**（基数 2 の Cooley–Tukey）。
> ほかの長さは `panic` します。0 埋めしてから渡してください。

> **注意: 速さより読みやすさを取っています。** 蝶形演算のたびに複素数を
...
注意: **matplotlib のコードは見ていません。** SVG は W3C の公開仕様で、
...
| 呼び出し | 中身 |
|---|---|
| `plot.Plot(w, h)` | 図を作る（`w`・`h` は 120 以上。注意: 小さすぎると panic） |
...
| `p.save(path)` | ファイルに書く（注意: `io` を使うのでベアメタル不可） |
...
注意: **幅に合わない値は、入れる時点で止まります**（黙って切り詰めません）。
...
注意: 64 ビットだけは符号つきのまま扱います（`int` がそれだからです）。
...
注意: **リンクの指定が要ります**（標準ライブラリには入っていません）。
...
注意: **渡しているあいだ、その list を変えないでください。** `append` で伸びると
別の場所へ移ることがあり、C 側が見ている番地が古くなります。

注意: 行列は**行優先の平たい `list[float]`** です（`linalg.Matrix` からは
