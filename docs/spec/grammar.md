...
(* 注意: 構文解析の時点で「str(式)」と桁揃えの呼び出しに書き換えます。
   ':' は**括弧の外側にあるもの**だけを見ます（xs[1:2] のスライスと区別）。 *)
fstring_item ::= "{" expr [ ":" format_spec ] "}"
format_spec  ::= [ fill ] [ "<" | ">" | "^" ] [ INT ] [ "." INT ] [ "f" ]

(* ── lambda（A-42）── *)
(* 注意: 本体は式 1 つだけです。型は書きません（置かれた場所から決まります）。
   外の変数は捕獲できます（A-43）。捕まえた値は作った関数の枠の上に置くので、
   捕獲した lambda は返せません・しまえません。 *)
