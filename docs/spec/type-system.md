...
yashirolang の型システムは、意図的にとても単純です。

| 性質 | yashirolang | 備考 |
|---|---|---|
| 静的 (static) | 済 | すべての型がコンパイル時に確定 |
| 明示的 (explicit) | 済 | 型注釈が必須。推論しない |
...
| 暗黙変換 (coercion) | なし | int↔float も明示変換のみ |
...
**注意: nullable にできるのは参照型（`str` / `list[T]` / class）だけです**。
...
| 入れ先 | 確かめるか |
|---|---|
| 変数の宣言・代入 / フィールドへの代入 | 済 |
| `list` の要素（リテラル・`append`・添字への代入） | 済 |
...
**注意: この検査は保守的**です。実際には到達しない経路でも `return` を要求します。
...
| yashirolang 型 | LLVM 型（値として） | LLVM 型（メモリ上） | 備考 |
|---|---|---|---|
| `int` | `i64` | `i64` | |
