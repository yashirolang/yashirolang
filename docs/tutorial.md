...
この言語は範囲外・桁あふれ・契約違反を**実行時に確かめます**。ただし
「必ず成り立つ」とコンパイラが示せたときは、その検査を出しません。
...
**書くものは 1 行も増えません。** 何が消えたかは `--prove-report` で見られます。
⚠️ 示せなかったところは今までどおり実行時に確かめます（安全性は変わりません）。
...
`-g` を付けると、デバッガと `perf` が**ソースの行**を出せるようになります。
⚠️ 変数の中身（`p x`）はまだ見られません（止める・たどる・測るまでです）。
