...
xs.remove(1)              # ⚠️ **添字**で消します（Python は値）
xs.reverse()
ys: list[int] = xs.copy()
xs.clear()
```
...
| 実数の除算 | `7.0 / 2.0` → `3.5`。⚠️ **`x / 0.0` は実行時エラー**（無限大は `math.inf()`） |
...
> ⚠️ f-string に書式指定（`f"{x:>8.2f}"`）はありません。桁を揃えるときは
...
⚠️ **クロージャと `lambda` はありません。** 状態を持たせたいときはインタフェース（§5.6）を使います。
...
⚠️ 型引数は引数の型から推論します。**戻り型にしか現れない型引数は書けません。**
...
⚠️ **型引数に境界（`T: Comparable` のようなもの）は書けません。** 使えるかは実体化のときに分かります。
...
| `x in c` | `__contains__`（⚠️ これだけ**右辺**の型で決まります） |

⚠️ **反転（`__radd__`）はありません。** どのメソッドが呼ばれるか読んで分かるように、
...
⚠️ **`self` も同じです。** 自分のフィールドを書き換えるメソッドは `def f(mut self, …)` と書きます。
...
⚠️ 循環参照は解放されません（弱参照はまだありません）。**必要なところだけ**に使ってください。
...
> ⚠️ 逃げ道として `--warn-own`（指摘を警告に落とす）がありますが、
...
⚠️ どちらの検査も、コンパイラが「必ず成り立つ」と示せたときは出力から消えます（§11）。
...
⚠️ **`Send` / `Sync` にあたるトレイトはありません。** 渡せるかどうかは
「借りか、所有か、`rc` か」で決まるので、**利用者が書く注釈は 1 つも増えません**。
⚠️ **`async` / `await` はありません**（理由は [design/concurrency.md](design/concurrency.md) §6）。
...
| `--warn-own` | 所有権の指摘を警告に落とす（⚠️ 保証の外に出ます） |
...
| クロージャ・`lambda` | トップレベルの関数を渡す（§4.3）／状態が要るならインタフェース |
