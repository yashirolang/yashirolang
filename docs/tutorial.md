...
f-string には**書式指定**も書けます（`[埋め文字][< > ^][幅][.桁数][f]`）。

```python
print(f"{name:<10}{price:>8.2f}")     # 表の 1 行
print(f"{x:*^9}")                     # ***42***
```

桁は**表示幅**で数えます（全角は 2 桁）。既に幅を超えていたら、そのまま出します。
小数の桁数（`.2f`）は `float` にだけ使えます。
...
```python
n: int = 2
print(count_if([1, 2, 3], lambda x: x > n))     # → 1
```

捕まえられるのは**値型だけ**（`int` / `float` / `bool` / 列挙 / 範囲型）で、
捕獲は**作ったときの値の写し**です。捕まえたあとに外を書き換えても、作り済みの
lambda は変わりません。

注意: **捕獲した lambda は、作った関数より長生きできません。**

```python
def make(n: int) -> fn(int) -> bool:
    return lambda x: x > n      # ✗ 返せません
```

捕まえた値は**その関数の枠の上**に置いてあります（確保しません）。だから
「返す」「フィールドやグローバルにしまう」「`spawn` に渡す」はコンパイル時に
断ります。**渡して呼んでもらうのはできます**（上の `count_if` がそれです）。

しまえるものが要るときは、インタフェース（§5.6）を使います。
...
| 捕獲した `lambda` を**しまう**こと | インタフェース（§5.6）。`lambda` と捕獲は §4.2 にあります |
| 入れ子のパターン・ガード（`case C(P(x)) if …`） | 本体で `match` を重ねる／`if` を書く |
