...
typedef enum {
    ...
    // ⚠️ 追加は**末尾に**。selfhost/ast が値を明示しているためです。
    ...
    //   ⚠️ 値はそのまま素通しします。変わるのは「外れていたら止まる」ことだけ。
    ND_RANGECHK,
    ...
    //   ⚠️ 箱（alloca）は作りません。**いま返そうとしている値**を指すだけです
...
typedef enum {
    ...
    // ★ 所属の検査。⚠️ **OP_EQ〜OP_GE の連続には入れません**
...
struct Node {
    ...
    //   ⚠️ --dump-ast には出しません（積み上げた AST 比較を壊さないため）。
