...
# 注意: **Windows では ws2_32 を明示的にリンクします。**
...
for f in "${FILES[@]}"; do
    ...
    #
    #   注意: これが無いと、`# FLAGS: --no-drop` と書いてあるケースを
    ...
    #
    #   注意: **`--deny-*` は混ぜません。** あれは診断を出すか出さないかだけの
    ...
    case_flags=""
    deny_case=0
    #   注意: `eval` は使わず 2 行読みます（FLAGS の中身を引用符として
    ...
    #
    # 注意: `# STAGE0-ONLY:`（run_tests.sh 用）とは別の印です。所有権のテストは
    ...
    # ── ② AST──
    #
    # 注意: 構文エラーのファイルは S 式を比べられない。位置だけ比べる。
    ...
    #   注意: import を含むファイルは入口として型検査できないもの（main が無い
    ...
    #
    #   注意: 診断の指定なので $case_flags には混ぜられず（混ぜると検査で
    ...
    # 注意: 万一無限ループしても比較全体を止めないよう、時間を区切る
    # 注意: 標準入力は **必ず /dev/null に繋ぎます**。繋がないと端末や CI の
...
#
# 注意: CRLF のファイルを tests/cases に置いても、git の改行変換で
