...
#
#   注意: これは「名前を探して置換する」道具ではありません。**書き漏れを見つけて
...
#
#   注意: これで `build/` `tests/tmp/` `bench/out/` のような**生成物が自動で
...
for pair in "LANG_NAME|$NAME" "LANG_CC|$CC" "LANG_PM|$PM" "LANG_EXT|$EXT"; do
    ...
    while IFS= read -r -d "" f; do
        f="./$f"
        allowed "$f" && continue
        # 注意: テキストファイルだけを見ます（実行ファイルに偶然並ぶバイト列は無視）
        LC_ALL=C grep -qI . "$f" 2>/dev/null || continue
        # 注意: ただの grep では足りません。旧パッケージマネージャ名が `pen` だった
        #   とき `grep -F pen` は `append` に当たりました。語の切れ目で探します。
        #
        # 注意: 拡張子を、この言語で書かれたソースの中で探すときは
