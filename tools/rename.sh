...
#
#   注意: 昔の版はリポジトリ全体を一括置換していましたが、**テストの中の
#     ただの文字列**（`Item("pen")` のような試験データ）まで書き換えてしまう
#     ため、やめました。置換しないことが安全です。
#
# 注意: 走らせたあとは必ず `make clean && make test && make bootstrap` を
...
# ── ① ソースファイルの拡張子を変える ──────────────────────
#   注意: .git と build は触りません。git 管理下のものは git mv を使うので
#     履歴が「改名」として残ります。
...
# ── ② 名前が決まる 3 か所を書き換える ──────────────────────
# 注意: 値は**環境変数で**perl に渡します。s/…/…/ の中に直接埋めると、
...
★ 名前を変えました。仕上げに次を通してください:
  ...
  注意: 2 つの langinfo がずれていると selfhost-test が落ちます。
